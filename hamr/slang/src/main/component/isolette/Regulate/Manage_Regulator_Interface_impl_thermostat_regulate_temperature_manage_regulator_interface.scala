...
import org.sireum._
import isolette.Isolette_Data_Model.{Failure_Flag_impl, Temp_impl}
import isolette._
...
object Manage_Regulator_Interface_impl_thermostat_regulate_temperature_manage_regulator_interface {
  ...
  def initialise(api: Manage_Regulator_Interface_impl_Initialization_Api): Unit = {
    ...
    api.put_upper_desired_temp(Temp_impl(InitialValues.DEFAULT_UPPER_DESIRED_TEMPERATURE))
    // set initial lower desired temp
    api.put_lower_desired_temp(Temp_impl(InitialValues.DEFAULT_LOWER_DESIRED_TEMPERATURE))
    // set initial displayed temp
    api.put_displayed_temp(Temp_impl(InitialValues.DEFAULT_DISPLAYED_TEMPERATURE))
    ...
    api.put_interface_failure(Failure_Flag_impl(InitialValues.DEFAULT_REGULATOR_INTERFACE_FAILURE_FLAG))
  }
